...
Knowing how to use both the `graphics` and `ggplot2` packages is worthwhile, so we denote one chapter to `ggplot2` and a second chapter to `graphics`. You'll notice that the chapters are essentially the same, producing the same graphs (with a few exceptions) to let you decide which graphing style you prefer. We are only requiring you to read one of the graphing chapters, and you can complete the associated exercises with either graphics package you like (or a combination of the two). We simply want to present you with both sets of tools so that in the future when you have graphs to produce you can use whichever package floats your boat! For now we'll start off with `ggplot2` and get to `graphics` in Chapter \@ref(graphics).
