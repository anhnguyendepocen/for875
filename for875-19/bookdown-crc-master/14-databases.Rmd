...
weather %>% 
  dplyr::select(thunder, mean_wind_speed) %>% 
  head(10)
```
...
```{r, tidy = FALSE}
weather %>% 
  dplyr::filter(mean_temp > 60 & mean_wind_speed > 10) %>% 
  summarize(count = n())
```
...
```{r, tidy = FALSE}
weather %>%
  dplyr::select(year, mean_wind_speed) %>% 
  dplyr::filter(year < 1940 & !is.na(mean_wind_speed)) %>% 
...
weather %>%
  dplyr::select(year, mean_wind_speed) %>% 
  dplyr::filter(year == 1929 & !is.na(mean_wind_speed)) %>% 
  dbplot_histogram(mean_wind_speed)
```
