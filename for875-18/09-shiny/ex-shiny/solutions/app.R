...
sites <- read.csv("http://msu.edu/~finleya/data/Michigan_State_Park_Campgrounds.csv")
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    ...
    if(nrow(sites1) > 0){
      plt <- plt + geom_point(data = sites1,aes(x=Long,y=Lat, size = TOTAL_SITE, color = COUNTY)) 
    } 
