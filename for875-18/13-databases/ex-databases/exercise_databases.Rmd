...
First we need to set up an area on our hard drive where we can store our database. Set the variable `dbdir` to some folder path on your computer (e.g., my directory is called `myDBDir` and is in the given working directory). Since we are going to create a new database, you should write this directory so the final directory in the path is an empty folder. We will then connection to the `MonetDBLite` database using the `dbConnect()` function from the `DBI` package. 
...
Next, we will copy multiple data frames from the `Lahman` package that contains information on over 10,000 baseball players. (Note that this not a typical practice. If you have data that is already stored in a data frame you should keep it in a data frame (unless it is so large it is slowing down your computations). It is not common practice to take a data frame and copy it over into an external database, but this is suitable for the purposes of this exercise.)
...
Take notice of how the `dbWriteTable` is used to add a new table to an existing database. This is one method of inserting records to a database (among many others). To see what tables our database now holds we run the following command:
